test_that("basic website example works", {
  ...
  expect_equal(app$execute_script("return window.greeting_count"), 0)
  expect_equal(app$execute_script("return window.custom_count"), 0)

  app$click(output = "greeting")
  app$click(output = "greeting")

  expect_equal(app$execute_script("return window.greeting_count"), 2)
  expect_equal(app$execute_script("return window.custom_count"), 0)
  ...
  expect_equal(app$execute_script("return window.greeting_count"), 2)
  expect_equal(app$execute_script("return window.custom_count"), 3)
})
