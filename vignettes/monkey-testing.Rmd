...
As `unleash` is a promise, we can execute a function right after, to check whether the script ran as expected.
...
If you run on CI/CD, don't forget to clean the session after tests are completed, particularly if you fixes the `shiny.port` option (you can't have 2 apps running on the same port):
