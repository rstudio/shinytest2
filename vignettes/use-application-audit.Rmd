...
code is defined below and located in [`vignettes/non-optimized-app/app.R`](https://github.com/rstudio/shinytest2/tree/main/vignettes/non-optimized-app/app.R). 
This app simulates a stiff oscillator, also known as Van der Pol [model](https://en.wikipedia.org/wiki/Van_der_Pol_oscillator). Under the hoods, 
this system composed of 2 differential equations is integrated with the 
`{deSolve}` package. Stiff systems need much smaller time step than
...
As a first test, you can run the app locally and notice how slow it is to 
...
resulting app response. It allows to answer many questions, such as:
...
simple: since R is single threaded, it can only use one processor at a time. 
In practice, this means that tasks are run sequentially. 
...
Below shows the code to start a Shiny app located at `path` on a specific port 
...
the app can't start. To launch the app and recorder we simply call:
...
We ensure to set a consistent load timeout, which will avoid to see this kind of 
annoying `{shinytest2}` warning:
...
- Install R, some system dependencies and R packages. Install Chrome and 
shinycannon tools for the headless loadtest.
...
jobs:
  shiny-loadtest-ci:
    ...
    steps:
      ...
      - name: Run load test 🏥
        shell: Rscript {0}
        run: source("R/audit-app.R"); record_loadtest(path = "app.R");
...
# Inside the project
workflows_path <- file.path(path, ".github/workflows")
r_path <- file.path(path, "R")
dir.create(workflows_path, recursive = TRUE)
dir.create(r_path)
# Copy all R helpers
file.create(file.path(r_path, "audit-app.R"))
# Copy the previous yaml template inside this file
file.create(file.path(workflows_path, "shiny-loadtest-ci.yaml"))
...
An example is available [here](https://divadnojnarg.github.io/RtmpXU8dBM/).
...
the app is clearly not able to handle 5 simultaneous user:
