...
Have you ever dreamt of profiling, load testing your Shiny app at each commit, without having to manually 
run any script?

In this vignette, we'll see how one can design and automate Shiny apps audit pipelines
with `{shinytest2}`.  

## Introduction

This case study consists in analyzing a particularly non-optimized app, given by the 
following code. This app simulates a stiff oscillator, also known as Van der Pol [model](https://en.wikipedia.org/wiki/Van_der_Pol_oscillator). Under the hoods, 
this system composed of 2 differential equations is integrated with the `{deSolve}` package:
...
As a first test, you can run the app locally and notice how slow it is to perform one 
computation. The next question is, how does this app scale? The answer is likely "No it doesn't", but
we would like to know exactly how bad it is. This is where load testing comes to play.

## Load testing with {shinytest2}

Shiny apps [load testing](https://rstudio.github.io/shinyloadtest/index.html) 
consists of running multiple user in parallel and measure the app response:
...
Many of the above questions can find answers thanks to `{shinyloadtest}`. A load test
is composed a three phases:

- Similarly to `{shinytest2}`, `{shinyloadtest}` records a user session with `shinyloadtest::record_session()`.
- This session is subsequently replayed by a Java-based tool, `shinycannon`, able to simulate multiple sessions
during a specific amount of time. 
- Sessions are analyzed by `shinyloadtest::load_runs()` and an HTML report generated with 
`shinyloadtest::shinyloadtest_report()`.

In general, the first step is done manually, that is, you play with the app as if you were a real business user
and stop the session when satisfied. However, because of `{shinytest2}` headless capabilities, we could
manipulate the app with `{shinytest2}` helpers such as `set_inputs` or raw 
JavaScript code to achieve the same goal. 

There is a tiny technical subtility to overcome. By default, `{shinytest2}` starts the app on a given port and the headless browser (Chrome) is then connected to the same port. This would be an issue with
`{shinyloadtest}` since the recorder does not listen to the same port, which is `8600`. 
In practice, we'll have to:

- Start the Shiny app as a background process on a given port. 
- Fire the load test recorder.
- Connect Chrome to the recorder on port `8600`. 

### Launch the background app

We leverage the `{callr}` package which exposes a convenient API to start R processes in the
background, that is, without blocking the main R process. Below shows the code to start
a Shiny app located at `path` on a specific port and run the load test recorder on the same port:
...
We can pass this function to the `start_r_bg()` function:
...
`r_bg()` starts a background R process passing the corresponding function and parameters.
We provide some log elements and safety guard in case the app couldn't start. 
To launch the app and recorder we simply call:

``` r
target <- start_r_bg(shiny_bg, path = "vignettes/non-optimized-app/app.R")
...
The previous part was the most technical step. Now, we only have to start a Chrome headless
browser on port `8600`, where the load test recorder runs. You'll notice that `{shinytest2}`
also supports remote urls. We ensure to set a consistent load timeout which would avoid to see a `{shinytest2}` warning:
...
chrome <- shinytest2::AppDriver$new("http://127.0.0.1:8600", load_timeout = 15000)
```

Below shows the list of R processes running so far in the background:
...
If everything is successful, you should see a __Client connected__ message in the R console. 
We then change some inputs without forgeting the timeout:

``` r
chrome$set_inputs(mu = 4, timeout_ = 15000)
```
...
Once satisifed, we may close the headless connection to stop the recorder:
...
If you see any weird error message in the shinycannon log, we suggest to re-run the
recording session and try again. 
...
Now, it is time to do all of this in CI/CD. For convenience, we wrap all the previous step in a single function:
...
Below is the necessary GitHub Actions yaml file:
...
jobs:
  shiny-loadtest-ci:
    ...
    steps:
      ...
      - name: Run load test 🏥
        shell: Rscript {0}
        run: devtools::load_all(); record_loadtest(path = "vignettes/non-optimized-app/app.R");
