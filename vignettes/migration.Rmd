...
old$click(".custom-selector", iotype = "auto")

## New
new$click("mybutton"); new$click(input = "mybutton")
new$click(output = "clickcount")
new$click(selector = ".custom-selector")
```
...
* `old$expectUpdate(output)`: This method is no longer supported. While knowing that an output value has been updated, it is very uncertain as to what the new value is. Equivalent code has been provided below.
...
* `old$getValue(name, iotype)` and `old$setValue(name, value, iotype)`: These methods have been removed as they were not generic and similar behavior achieved via `new$get_values(input = name)$input` and `new$set_values(name = value)`.
...
TODO-barret; Should we export this method? No?
* `old$checkUniqueWidgetNames()`: Automatically performed on startup in `{shinytest2}`.

* `old$clone()`: `AppDriver` does not support cloning. (The underlying `ChromoteSession` does not support cloning, so `AppDriver` does not support cloning.)
...
## New
path <- new$get_path()
filename <- if (fs::dir_exists(path)) "" else basename(path)
