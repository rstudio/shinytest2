...
In some cases there might be a need to provide the input components using the variables (not hard-coded). One can use the `inputs` parameter, expected to be the `rlang::list2` object with component-value pairs.
```{r}
