...
For an indepth discussion of expectations, see the [expectations vignette](expectations).
TODO-barret
...
If your test script has already been created, you can set the random seed by editing the test script so that the `AppDriver$new()` call has a value for `seed`. Moreover, if you need these tests to pass on multiple versions of R with different `RNGkind()` defaults (e.g., 3.5 and 3.6), you may want to set a fixed `RNGkind()` across your tests (perhaps via `RNGversion("3.5.0")`).
<!-- The **shinytest2** package does this in its own automated tests [via a **testthat** helper](https://github.com/rstudio/shinytest2/tree/main/tests/testthat/test-helper-rng.R). -->
...
Before you run the test script, you must copy the file to the `tests/testthat/` directory.  See the `tests/testthat/` subdirectory of [this app](https://github.com/rstudio/shinytest/tree/main/tests/testthat/recorded_tests/009-upload) for an example.
...
Another way is to take a screenshot of your application. You can call `app$get_screenshot()` to take a screenshot of the application. This will display the screenshot as if it were a plot. (In RStudio, it will show in the Viewer pane.)
...
Learn about using shinytest2 with [continuous integration](ci.html).
TODO-barret; update link
