...
However, `{shinytest2}` does not do this for code brevity and relies on `AppDriver$private$finalizer()` (which is called when the value is garbage collected or the R session is stopped) to call `AppDriver$stop()`.

If your R session is starting to feel more sluggish, you may want to call `app$stop()` manually. Or if you are programattically creating many instances of `AppDriver`, you may want to call `app$stop()` when the specific test is compelte.
