...
When testing Shiny applications within R packages, you may need access
to helper functions, modules, or other objects defined in your app’s
`./R` directory. The
...
functions help manage this.

### When do you need these functions?

**You do NOT need them if:**

- Your app is defined inline (as a Shiny app object passed directly to
  `AppDriver$new()`)
- All needed functions are already available from your package via
  [`library(yourpackage)`](https://rdrr.io/r/base/library.html) or in
  the test file itself

**Example - inline app (no
[`local_app_support()`](https://rstudio.github.io/shinytest2/reference/app_support.md)
needed):**
...
- Your app is stored in a directory (e.g., `inst/myapps/app1/`)
- The app has supporting files in its own `./R` subfolder
- You need to test those supporting functions or access them in your
  tests

### Which function should you use?

**`local_app_support(app_dir)`** - Recommended for package testing

- Automatically attaches the app’s support environment for the current
  test
- Automatically cleans up when the test completes
- Prevents conflicts between different app tests
...
**`with_app_support(app_dir, { ... })`** - For explicit scoping

- Support environment is only available within the expression block
- Cleaned up immediately after the expression completes
- Useful when you want very clear boundaries
...
**`load_app_support(app_dir)`** - For standalone apps only

- Permanently loads support files into the testing environment
- No automatic cleanup
- Best used in `setup-shinytest2.R` for non-package Shiny applications
- **Not recommended for package testing** as it can cause conflicts

### Benefits of using these functions

1.  **Isolation**: Tests for different apps don’t interfere with each
    other
2.  **Access to internals**: Test helper functions and modules without
    manual sourcing
3.  **Clean testing**: Automatic cleanup prevents pollution of test
    environment
4.  **Package compatibility**: Works seamlessly with package development
    workflow
